...
LLM_MODEL = "gemma-3-1b-it-Q8_0"                          #LLM모델
...
st.set_page_config(page_title="강미랑", layout="wide")
...
# --- 3. 사이드바 구성 ---
    with st.sidebar:
        ...
        k_value = st.slider(f"{'&nbsp;' * 5}⤷ 검색할 문서 청크 개수 (기본: {DEFAULT_K_VALUE})",
                            min_value=5, max_value=40, value=DEFAULT_K_VALUE)
...
############  관리자 role 만 사용 
    ...
    st.write(f"[사업관리 RAG 사용 모델 /*:&nbsp; 🧠LLM 모델:&nbsp; {LLM_MODEL}&nbsp;&nbsp;  🧮임베딩 모델:&nbsp; {EMBEDDING_MODEL}:&nbsp; 💡유사도: 벡터거리*/ ]")
